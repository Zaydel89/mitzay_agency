  ...
  return (
    <footer className="w-full glass p-8 sm:p-12 rounded-[2.5rem] sm:rounded-[3rem] border border-primary/20 text-center relative overflow-hidden">
      <div className="relative z-10">
        ...
        <p className="text-gray-400 text-[0.625rem] sm:text-xs max-w-xs mx-auto mb-8 sm:mb-10">{FOOTER_CONTENT.microcopy}</p>

        <div className="flex flex-wrap justify-center gap-3 sm:gap-4 mb-8 sm:mb-10">
          {FOOTER_CONTENT.socials.map(s => (
            <a key={s.name} href={s.url} className="px-4 sm:px-5 py-2 glass rounded-full text-[0.5rem] sm:text-[0.5625rem] font-black uppercase tracking-widest text-gray-400 hover:text-primary transition-colors">{s.name}</a>
          ))}
        </div>

        <div className="pt-6 sm:pt-8 border-t border-white/10">
          <p className="text-[0.5rem] sm:text-[0.5625rem] text-gray-500 font-bold uppercase tracking-[0.2em]">{FOOTER_CONTENT.legal}</p>
        </div>
      </div>
    </footer>
  );
